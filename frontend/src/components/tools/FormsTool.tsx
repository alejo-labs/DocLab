...
import { FormInput, MousePointer2, Type, Pilcrow, CheckSquare, CircleDot, ChevronDownSquare, ChevronDown, Trash2, Copy, Wand2, Download, Eye, Pencil, PenLine, FileDown, List, LayoutPanelTop, Bold, Italic, AlignLeft, AlignCenter, AlignRight, ZoomIn, ZoomOut, Plus, X } from 'lucide-react';
...
function DesignPanel({ bytes, designKind, setDesignKind, placed, setPlaced, draft, setDraft, counter, onExport, onApplyFill, busy }: DesignPanelProps) {
  ...
  return (
    <div className="flex flex-col-reverse gap-4 lg:h-full lg:flex-row lg:items-stretch lg:overflow-hidden">
      <div className="flex min-w-0 flex-1 flex-col rounded-[var(--radius-instrument)] border border-line bg-paper-raised p-3 lg:h-full lg:overflow-hidden">
        ...
        <div className="flex flex-col items-center gap-6 pb-8 lg:min-h-0 lg:flex-1 lg:overflow-y-auto lg:overflow-x-auto lg:pr-1">
          {pages.map((page, pageIndex) => {
            const s = page.displayScale * zoom;
      ...
      {/* Panel lateral */}
      <aside className="space-y-3.5 rounded-[var(--radius-instrument)] border border-line bg-paper-raised p-3.5 lg:max-h-full lg:w-80 lg:shrink-0 lg:overflow-y-auto xl:w-96">
        <div>
          <p className="mb-2 flex items-center gap-1.5 font-mono text-xs font-bold uppercase tracking-wider text-signal-deep">
            <Wand2 className="size-3.5" aria-hidden /> Herramienta
          </p>
          <div className="grid grid-cols-3 gap-1.5">
            {TOOLS.map(({ k, icon: Icon, label }) => (
              <button
                key={k}
                type="button"
                onClick={() => { setDesignKind(k); if (k !== 'select') setSelectedId(null); }}
                className={`flex flex-col items-center gap-1.5 rounded-lg border p-2.5 text-[11px] transition-all ${
                  designKind === k
                    ? 'border-signal-deep bg-signal/15 text-signal-deep font-semibold shadow-xs ring-1 ring-signal/30'
                    : 'border-line/80 bg-paper/60 text-ink/80 hover:border-signal/50 hover:bg-paper hover:text-ink font-medium'
                }`}
              >
                <Icon className={`size-4 ${designKind === k ? 'text-signal-deep' : 'text-graphite'}`} aria-hidden />
                {label}
              </button>
            ))}
          </div>
        ...
        {selected ? (
          ...
          <div>
            <p className="mb-2 flex items-center gap-1.5 font-mono text-xs font-bold uppercase tracking-wider text-signal-deep">
              <List className="size-3.5" aria-hidden /> Campos ({placed.length})
            </p>
            {placed.length === 0 ? (
              ...
              <ul className="space-y-1">
                {placed.map((f) => {
                  ...
                  return (
                    <li key={f.id}>
                      <button type="button" onClick={() => { setDesignKind('select'); setSelectedId(f.id); }} className="flex w-full items-center gap-2 rounded-md border border-line bg-paper/50 px-2.5 py-1.5 text-left text-sm font-medium text-ink hover:border-signal/50 hover:bg-paper">
                        <Icon className="size-3.5 shrink-0 text-signal-deep" aria-hidden />
                        <span className="min-w-0 flex-1 truncate">{f.name}</span>
                        {f.required && <span className="text-ember font-bold">*</span>}
                      </button>
                    </li>
                  );
                })}
        ...
        <div className="space-y-2 pt-1">
          <Button onClick={onExport} loading={busy} disabled={!placed.length} className="w-full font-semibold shadow-sm"><FileDown className="size-4" aria-hidden /> Descargar formulario</Button>
          <button type="button" onClick={onApplyFill} disabled={!placed.length || busy} className="inline-flex w-full items-center justify-center gap-1.5 rounded-[var(--radius-instrument)] border border-signal/40 bg-signal/10 py-2.5 text-sm font-semibold text-signal-deep hover:bg-signal/20 disabled:opacity-50 transition-colors"><Wand2 className="size-4" aria-hidden /> Aplicar y rellenar aquí</button>
        </div>
...
function OptionTagsInput({ options, onChange }: OptionTagsInputProps) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1.5 rounded-lg border border-line bg-paper/60 p-2 min-h-11 items-center">
        {options.length === 0 ? (
          <span className="font-mono text-[11px] text-graphite italic">Sin opciones aún</span>
        ) : (
          options.map((opt, i) => (
            <span
              key={`${opt}-${i}`}
              className="inline-flex items-center gap-1.5 rounded-md border border-signal/40 bg-signal/15 px-2 py-0.5 font-mono text-xs font-semibold text-signal-deep shadow-2xs"
            >
              <span>{opt}</span>
              <button
                type="button"
                onClick={() => remove(i)}
                className="grid size-3.5 place-items-center rounded-full text-signal-deep/70 hover:bg-ember/20 hover:text-ember transition-colors"
                title={`Eliminar "${opt}"`}
      ...
      <div className="flex gap-1.5">
        ...
        <button
          ...
          className="inline-flex items-center gap-1 rounded-md border border-signal-deep bg-signal-deep px-2.5 py-1 font-mono text-xs font-semibold text-paper shadow-2xs hover:bg-signal-deep/90 disabled:opacity-40 transition-all shrink-0"
        >
...
function PropertyPanel({ f, update, onDuplicate, onRemove }: PropertyPanelProps) {
  ...
  return (
    <div className="space-y-3.5">
      <p className="flex items-center gap-1.5 font-mono text-xs font-bold uppercase tracking-wider text-signal-deep">
        <LayoutPanelTop className="size-3.5" aria-hidden /> Propiedades del campo
      </p>
      ...
      {hasOptions && (
        <Section title="Opciones" defaultOpen>
          <Field label="Opciones de la lista">
            <OptionTagsInput options={f.options ?? []} onChange={(options) => update({ options })} />
          </Field>
        </Section>
      )}
      ...
      <Section title="Apariencia">
        <label className="block">
          <span className="mb-1 flex items-center justify-between font-mono text-[11px] font-medium text-ink/80">Grosor del borde <span className="font-bold text-signal-deep">{f.borderWidth ?? 1}px</span></span>
        ...
        <div className="flex items-center gap-3 pt-1">
          <label className="flex items-center gap-1.5 text-xs font-medium text-ink cursor-pointer">
            <input type="color" value={f.borderColor ?? '#0a8c81'} onChange={(e) => update({ borderColor: e.target.value })} className="size-6 cursor-pointer rounded border border-line bg-paper shadow-2xs" /> Borde
          </label>
          <label className="flex items-center gap-1.5 text-xs font-medium text-ink cursor-pointer">
            <input type="color" value={f.backgroundColor ?? '#ffffff'} onChange={(e) => update({ backgroundColor: e.target.value })} disabled={f.backgroundColor == null} className="size-6 cursor-pointer rounded border border-line bg-paper shadow-2xs disabled:opacity-40" /> Fondo
          </label>
          <label className="flex items-center gap-1.5 text-xs font-medium text-graphite cursor-pointer">
      ...
      {(f.kind === 'text' || f.kind === 'textarea' || f.kind === 'dropdown') && (
        <Section title="Texto">
          ...
          <div className="flex items-center gap-2 pt-1">
            <button type="button" onClick={() => update({ bold: !f.bold })} aria-pressed={!!f.bold} title="Negrita" className={`grid size-7 place-items-center rounded border transition-colors ${f.bold ? 'border-signal bg-signal/20 text-signal-deep font-bold' : 'border-line bg-paper/50 text-graphite hover:text-ink'}`}><Bold className="size-3.5" /></button>
            <button type="button" onClick={() => update({ italic: !f.italic })} aria-pressed={!!f.italic} title="Cursiva" className={`grid size-7 place-items-center rounded border transition-colors ${f.italic ? 'border-signal bg-signal/20 text-signal-deep font-bold' : 'border-line bg-paper/50 text-graphite hover:text-ink'}`}><Italic className="size-3.5" /></button>
            <label className="ml-1 flex items-center gap-1.5 text-xs font-medium text-ink cursor-pointer">
              <input type="color" value={f.textColor ?? '#14161b'} onChange={(e) => update({ textColor: e.target.value })} className="size-6 cursor-pointer rounded border border-line bg-paper shadow-2xs" /> Color
            </label>
            {(f.kind === 'text' || f.kind === 'textarea') && (
              <div className="ml-auto inline-flex rounded-md border border-line bg-paper/60 p-0.5">
                {([['left', AlignLeft], ['center', AlignCenter], ['right', AlignRight]] as const).map(([a, Icon]) => (
                  <button key={a} type="button" onClick={() => update({ align: a })} title={a} className={`grid size-6 place-items-center rounded transition-colors ${(f.align ?? 'left') === a ? 'bg-ink text-paper font-semibold' : 'text-graphite hover:text-ink'}`}><Icon className="size-3.5" /></button>
                ))}
              </div>
            )}
          </div>
      ...
      <div className="flex gap-2 pt-1">
        <button type="button" onClick={onDuplicate} className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-md border border-line bg-paper/80 py-2 text-xs font-semibold text-ink shadow-2xs hover:border-signal/50 hover:bg-signal/5 transition-colors"><Copy className="size-3.5" /> Duplicar</button>
        <button type="button" onClick={onRemove} className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-md border border-ember/40 bg-ember/15 py-2 text-xs font-semibold text-ember hover:bg-ember/25 transition-colors shadow-2xs"><Trash2 className="size-3.5" /> Borrar</button>
      </div>
    </div>
  );
}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1 block font-mono text-[11px] font-medium text-ink/80">{label}</span>
      {children}
    </label>
  );
}

/** Sección plegable del panel de propiedades (acordeón), para no saturar al usuario. */
function Section({ title, defaultOpen = false, children }: { title: string; defaultOpen?: boolean; children: React.ReactNode }) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div className={`overflow-hidden rounded-lg border transition-colors ${open ? 'border-signal/40 shadow-xs' : 'border-line/80'}`}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className={`flex w-full items-center justify-between gap-2 px-3 py-2.5 text-left transition-colors ${
          open ? 'bg-signal/10 text-signal-deep' : 'bg-paper/70 hover:bg-paper text-ink'
        }`}
      >
        <span className="font-mono text-[11px] font-bold uppercase tracking-wider">{title}</span>
        <ChevronDown className={`size-4 shrink-0 transition-transform ${open ? 'rotate-180 text-signal-deep' : 'text-graphite'}`} aria-hidden />
      </button>
      {open && <div className="space-y-2 border-t border-line/60 bg-paper-raised/40 p-3">{children}</div>}
    </div>
  );
}
