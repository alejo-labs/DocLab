...
export function PageNumbersTool(_props: ToolEngineProps) {
  ...
  const overlay = (dims: { width: number; pageCount: number; scale: number }) => {
    const scale = dims.scale;
    const sample = formatNumber(template || '{n}', startAt, dims.pageCount);
    return (
      <div className="flex h-full w-full p-4 overflow-hidden" style={{ justifyContent: FLEX[position].justify, alignItems: FLEX[position].align }}>
        <span style={{ color, fontSize: Math.max(3, fontSize * scale), fontFamily: FONTS[font].css, fontWeight: 600, lineHeight: 1 }}>{sample}</span>
      </div>
    );
  };
    ...
    <div className="flex flex-col gap-6 lg:h-full lg:flex-row lg:items-stretch lg:overflow-hidden">
      ...
      <div className="min-w-0 flex-1 space-y-4 lg:h-full lg:overflow-y-auto lg:pr-3">
        ...
        <div className="flex flex-wrap items-start gap-6">
          <div>
            <p className="mb-1.5 font-mono text-xs font-bold uppercase tracking-wider text-signal-deep">Posición</p>
            <PositionGrid value={position} onChange={setPosition} />
          </div>
          <div className="flex-1 space-y-3">
            <label className="flex flex-col gap-1.5 text-sm">
              <span className="font-mono text-xs text-graphite">Formato de numeración</span>
            ...
            <div className="flex flex-wrap gap-1.5">
              {FORMATS.map((f) => (
                <button key={f} type="button" onClick={() => setTemplate(f)} className={`rounded-full border px-2.5 py-1 font-mono text-xs transition-colors ${template === f ? 'border-signal bg-signal/10 text-signal-deep font-bold' : 'border-line text-graphite hover:border-signal/40'}`}>{f}</button>
              ))}
            </div>
            <p className="font-mono text-[11px] text-graphite">Variables: {'{n}'} número · {'{total}'} total · {'{N}'} ceros iniciales</p>
          </div>
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <div><p className="mb-1.5 font-mono text-xs font-bold uppercase tracking-wider text-signal-deep">Color del texto</p><ColorControl value={color} onChange={(c) => setColor(c!)} /></div>
          <div><p className="mb-1.5 font-mono text-xs font-bold uppercase tracking-wider text-signal-deep">Tipografía</p><Segmented value={font} onChange={setFont} options={(Object.keys(FONTS) as FontFamily[]).map((f) => ({ value: f, label: FONTS[f].label }))} /></div>
        </div>
        <LabeledSlider label="Tamaño de fuente" value={fontSize} min={8} max={48} unit="pt" onChange={setFontSize} />

        <div className="flex flex-wrap items-end gap-4 pt-1">
          <label className="flex flex-col gap-1.5 text-sm">
            <span className="font-mono text-xs text-graphite">Empezar en página N.º</span>
            <input type="number" min={0} value={startAt} onChange={(e) => setStartAt(Number(e.target.value))} className="w-28 rounded-md border border-line bg-paper px-3 py-2 font-mono text-ink focus:border-signal focus:outline-none" />
          </label>
          <label className="flex flex-1 flex-col gap-1.5 text-sm">
            <span className="font-mono text-xs text-graphite">Omitir páginas (separadas por coma)</span>
            <input value={skipText} onChange={(e) => setSkipText(e.target.value)} placeholder="Ej.: 1, 2" className="rounded-md border border-line bg-paper px-3 py-2 font-mono text-sm text-ink focus:border-signal focus:outline-none" />
          </label>
        </div>
        <label className="flex items-center gap-2 text-sm text-ink font-medium cursor-pointer"><input type="checkbox" checked={countSkipped} onChange={(e) => setCountSkipped(e.target.checked)} className="size-4 accent-signal" /> Las páginas omitidas cuentan en la secuencia</label>

        <div className="flex items-center gap-3 pt-2">
          <Button onClick={run} loading={busy} className="font-semibold shadow-sm"><Hash className="size-4" aria-hidden /> Numerar páginas</Button>
          <button type="button" onClick={() => setBytes(null)} className="text-sm text-graphite hover:text-ink">Cambiar archivo</button>
        </div>
      </div>

      {/* Vista previa lateral derecha */}
      <div className="flex flex-col items-center justify-start rounded-[var(--radius-instrument)] border border-line bg-paper-raised p-4 lg:w-[440px] lg:shrink-0 lg:h-full lg:overflow-y-auto">
        <p className="mb-3 font-mono text-xs font-bold uppercase tracking-wider text-signal-deep w-full text-left">Vista previa en tiempo real</p>
        <LivePreview bytes={bytes} width={400} overlay={overlay} />
      </div>
    </div>
  );
}
