...
export function MergeTool(_props: ToolEngineProps) {
  ...
  useReportActiveDoc(pages.length > 0);

  const { thumbSize, setThumbSize, gridStyle } = useThumbnailSize(pages.length);
  ...
  const ensureThumb = useCallback(async (fileId: string, pageIndex: number) => {
    ...
    try {
      const t = await renderPageThumbnail(entry.pdf, pageIndex + 1, 280);
  ...
  // ── Lightbox HD: usa los proxies ya cargados para renderizar a alta resolución ──
  const renderHd = useCallback(async (idx: number) => {
    const p = pages[idx];
    if (!p) throw new Error('Página no encontrada');
    const entry = proxies.current.get(p.fileId);
    if (!entry) throw new Error('Proxy no disponible');
    const thumb = await renderPageThumbnail(entry.pdf, p.pageIndex + 1, 900);
    return thumb.dataUrl;
  }, [pages]);

  const lightboxPages: LightboxPage[] = pages.map((p, i) => ({
    label: `Pág. ${i + 1} — ${p.name}`,
    previewUrl: thumbs[thumbKey(p.fileId, p.pageIndex)] ?? '',
  }));
  ...
  return (
    <div className="space-y-5">
      {/* Dropzone grande solo hasta el primer archivo; luego una barra fina + soltar
          en las miniaturas para aprovechar el espacio. */}
      {pages.length === 0 ? (
        ...
        <FileDropzone accept="application/pdf" multiple compact hint="Arrastra más PDF aquí (o sobre las miniaturas) para añadirlos" onFiles={addFiles} />
      )}
      ...
      {pages.length > 0 && (
        <>
          ...
          <ul
            ...
            onDrop={(e) => { e.preventDefault(); setDragOver(false); const files = Array.from(e.dataTransfer.files); if (files.length) addFiles(files); }}
            style={gridStyle}
            className={`grid gap-3 rounded-[var(--radius-instrument)] lg:max-h-[calc(100vh-10rem)] lg:overflow-y-auto p-1 ${dragOver ? 'outline-2 outline-dashed outline-signal outline-offset-4' : ''}`}
          >
            {pages.map((p, index) => (
              <PageCard
                key={p.id}
...
function PageCard({ page, index, thumb, onVisible, onRotate, onRemove, onZoom }: PageCardProps) {
  ...
  return (
    <li ref={ref} data-id={page.id} className="page-handle group relative cursor-grab rounded-[var(--radius-instrument)] border border-line bg-paper-raised p-1.5 active:cursor-grabbing">
      ...
      <div className="relative grid aspect-[3/4] place-items-center overflow-hidden rounded bg-white">
        {thumb ? (
        ...
        {/* Botón de ampliar centrado sobre la miniatura */}
        <div className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity group-hover:opacity-100">
          <LightboxTrigger onClick={onZoom} />
        </div>
      </div>
