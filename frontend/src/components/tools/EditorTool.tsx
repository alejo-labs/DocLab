...
export function EditorTool(_props: ToolEngineProps) {
  ...
  const { pages, loading, error: loadError } = usePdfPages(bytes, Math.round(baseWidth * zoom));
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      const w = Math.max(320, Math.min(el.clientWidth - 8, 1000));
      setBaseWidth((p) => (Math.abs(p - w) > 30 ? w : p));
    });
    ro.observe(el);
  ...
  useEffect(() => {
    ...
    const applyMove = (el: HTMLElement, dx: number, dy: number) => {
      ...
      const scale = page?.displayScale ?? 1;
  ...
  return (
    <>
      ...
      <div className="flex flex-col-reverse gap-4 lg:flex-row lg:items-start">
        ...
        <div className="flex min-w-0 flex-1 gap-3">
          ...
          {pages.length > 1 && (
            <div className="hidden w-14 shrink-0 lg:block">
              <div className="sticky top-24 max-h-[calc(100dvh-7rem)] space-y-1.5 overflow-y-auto pr-1">
                {pages.map((p, i) => (
                  <button
                    key={p.pageNumber}
                    type="button"
        ...
        {/* Panel */}
        <aside className="rounded-[var(--radius-instrument)] border border-line bg-paper-raised p-3 lg:sticky lg:top-24 lg:w-72 lg:shrink-0">
          {/* Zoom + historial */}
  ...
  if (a.type === 'text') {
    return (
      <div key={a.id} {...common} onDoubleClick={(e) => { onActivate(a.id); (e.currentTarget.querySelector('textarea') as HTMLTextAreaElement | null)?.focus(); }} className={`ed-text group absolute ${isSel ? 'ring-1 ring-signal/60' : ''}`} style={{ left: a.x * s, top: a.y * s, transform: `rotate(${a.rotation}deg)`, transformOrigin: 'center', opacity: a.opacity, pointerEvents: clickable ? 'auto' : 'none' }}>
        {movable && <span className="ed-handle absolute -left-5 top-0 cursor-grab text-graphite opacity-0 group-hover:opacity-100"><GripVertical className="size-4" /></span>}
        <textarea value={a.text} onChange={(e) => onText(a.id, e.target.value)} rows={Math.max(1, a.text.split('\n').length)} cols={Math.max(8, ...a.text.split('\n').map((l) => l.length + 1))} placeholder="Escribe…" className="resize-none overflow-hidden bg-transparent leading-tight outline-none"
