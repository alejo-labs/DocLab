...
export function OnDeviceBadge({ processing, size = 'sm' }: OnDeviceBadgeProps) {
  ...
  // En tamaño compacto (tarjetas) el texto es más corto para no desbordar las tarjetas
  // estrechas; en las cabeceras (md) se muestra el mensaje completo.
  const compact = size === 'sm';

  if (isOnDevice) {
    return (
      <span
        className={`inline-flex max-w-full items-center gap-1.5 overflow-hidden whitespace-nowrap rounded-full border border-signal/40 bg-signal/10 font-mono font-medium tracking-wide text-signal-deep ${padding}`}
      ...
      >
        ...
        {compact ? 'ON-DEVICE · 0 B' : 'ON-DEVICE · 0 B SUBIDOS'}
  ...
  return (
    <span
      className={`inline-flex max-w-full items-center gap-1.5 overflow-hidden whitespace-nowrap rounded-full border border-ember/40 bg-ember/10 font-mono font-medium tracking-wide text-ember ${padding}`}
    ...
    >
      ...
      {compact ? 'SERVIDOR EFÍMERO' : 'SERVIDOR EFÍMERO · SIN RETENCIÓN'}
    </span>
  );
}
