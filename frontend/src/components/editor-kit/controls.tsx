...
export function ZoomControls({ zoom, onZoom, onFit }: { zoom: number; onZoom: (z: number) => void; onFit: () => void }) {
  ...
  return (
    <div className="flex items-center gap-1 rounded-md border border-line bg-paper p-0.5">
      <button type="button" onClick={() => onZoom(Math.max(0.25, zoom - 0.1))} title="Alejar" className="grid size-7 place-items-center rounded text-graphite hover:bg-ink/5 hover:text-ink">
        <Minus className="size-4" />
      </button>
      <span className="w-12 text-center font-mono text-xs text-ink">{pct}%</span>
      <button type="button" onClick={() => onZoom(Math.min(4, zoom + 0.1))} title="Acercar" className="grid size-7 place-items-center rounded text-graphite hover:bg-ink/5 hover:text-ink">
        <Plus className="size-4" />
      </button>
      <button type="button" onClick={onFit} title="Ajustar ancho" className="grid size-7 place-items-center rounded text-graphite hover:bg-ink/5 hover:text-ink">
