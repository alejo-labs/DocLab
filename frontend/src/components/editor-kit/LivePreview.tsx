...
interface PageInfo {
  ...
  height: number;
  pageCount: number;
  scale: number; // px por punto (para escalar overlays correctamente en cualquier tamaño de página)
}
 ...
 */
export function LivePreview({ bytes, width = 280, page = 1, overlay }: { bytes: Uint8Array | null; width?: number; page?: number; overlay?: (dims: { width: number; height: number; pageCount: number; scale: number }) => ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setInfo({ dataUrl: t.dataUrl, width: t.width, height: t.height, pageCount: pdf.numPages, scale: t.scale });
        await task.destroy();
      } catch {
    ...
    <div className="relative inline-block rounded-md border border-line bg-white shadow-sm" style={{ width: info?.width ?? width, minHeight: 80 }}>
      {info ? (
        <>
          ...
          {overlay && (
            <div className="pointer-events-none absolute inset-0">{overlay({ width: info.width, height: info.height, pageCount: info.pageCount, scale: info.scale })}</div>
          )}
