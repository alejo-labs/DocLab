...
const flow = [
  ...
  { n: '03', title: 'Descargas el resultado', body: 'El archivo generado se ofrece como descarga local (object URL). Al cerrar la pestaña no queda ni rastro: sin copias, sin servidor.' },
];
...
const engines = [
  ...
  { icon: Boxes, title: 'fflate · docx · pptx', body: 'Compresión ZIP y generación de documentos Office (Word/PowerPoint) para las conversiones desde PDF, también en el cliente.' },
  { icon: Wand2, title: 'Motor de estructura', body: 'Un pipeline propio (spans → líneas → bloques → tablas) que reconstruye la maquetación para PDF→Word con dos modos: fiel o texto limpio.' },
];
      ...
      <Prose>
        ...
        <Section title="El recorrido de un archivo">
          ...
          <p className="flex items-center gap-2 rounded-[var(--radius-instrument)] border border-signal/30 bg-signal/5 px-4 py-3 text-sm text-signal-deep">
            ...
            Puedes comprobarlo tú mismo: abre las herramientas de red del navegador (pestaña «Network»), procesa un archivo y verás que no se envía nada. Incluso funciona sin conexión una vez cargada la página.
          </p>
        </Section>
        ...
        <Section title="¿Por qué en el navegador?">
          <p>
            El modelo clásico —subir tu documento a un servidor, procesarlo allí y devolvértelo— implica confiar en que
            ese servidor no guarda copias, no lo indexa y lo borra de verdad. DocLab elimina esa confianza de la
        ...
        <Section title="Los motores bajo el capó">
          <p>
            Cada herramienta se apoya en librerías de código abierto (licencias permisivas MIT/Apache/ISC),
            <strong> autoalojadas</strong> en nuestro propio dominio para no depender de CDNs de terceros que pudieran
            observar tu actividad. Se cargan de forma diferida: solo bajas el motor de la herramienta que abres.
          </p>
          <div className="grid gap-4 sm:grid-cols-2">
            {engines.map((e) => (
        ...
        <Section title="Construido con">
          <p className="text-sm">
            React 19 · TypeScript · Vite · Tailwind CSS v4 en el frontend; un backend mínimo en Express (solo el proxy
            de búsqueda) endurecido con <Mono>helmet</Mono>, validación <Mono>zod</Mono> y limitación de peticiones.
            Servido tras Nginx con una <Mono>Content-Security-Policy</Mono> estricta. Todo el detalle en la página de{' '}
